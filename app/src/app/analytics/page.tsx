...
function ExpenseLog() {
  ...
  const sectors = [...new Set(expenses.map(e => e.sector_name).filter(Boolean))];
  const types = [...new Set(expenses.map(e => e.expense_category).filter(Boolean))];
...
function Weather() {
  ...
  return (
    <div>
      <SectionCard title="الطقس الحالي — قاقوس, الشرقية" subtitle="">
